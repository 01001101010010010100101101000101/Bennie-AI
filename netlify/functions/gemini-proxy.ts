...
// Types duplicated from types.ts to avoid pathing issues in Netlify build.
enum Sender {
  User = 'user',
  Bennie = 'bennie',
}
...
const handler: Handler = async (event: HandlerEvent) => {
    ...
    if (!API_KEY) {
        return { statusCode: 500, body: JSON.stringify({ error: "API_KEY environment variable not set." }) };
    }
    ...
    try {
        ...
        return {
            statusCode: 500,
            body: JSON.stringify({ error: `An error occurred while processing your request: ${errorMessage}` }),
        };
    }
};
